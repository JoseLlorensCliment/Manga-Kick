...
2.  **Persistencia Limitada a la Sesión (Base de Datos en Memoria):** Para maximizar la velocidad de respuesta, simplificar el despliegue del entorno y evitar la sobrecarga que supone configurar un motor de base de datos externo (como PostgreSQL o MongoDB) en máquinas cliente, la aplicación utiliza una base de datos interna estructurada en memoria del servidor Express. Toda alteración de los datos (subidas de nivel por entrenamiento, compras del draft) se mantiene activa mientras el servidor esté en ejecución. La persistencia definitiva de cuentas de usuario en una base de datos relacional/documental persistente queda postergada para futuras versiones.
